...
class Solver(object):
    ...
    def train(self):
        ...
        for i in range(start_iters, self.num_iters):
            ...
            try:
                x_real, x_style, x_char, y_trg, y_style, y_char = next(data_iter)
            except:
                data_iter = iter(data_loader)
                x_real, x_style, x_char, y_trg, y_style, y_char = next(data_iter)
            #pdb.set_trace()
            ...
            if self.dataset == 'CelebA':
                ...
                c_org = self.label2onehot(x_char, self.c_dim)
                c_trg = self.label2onehot(y_char, self.c_dim)
            
            x_real = x_real.to(self.device)           # Input images.
            y_trg=y_trg.to(self.device)
            c_org = c_org.to(self.device)             # Original domain labels.
            c_trg = c_trg.to(self.device)             # Target domain labels.
            x_char = x_char.to(self.device)     # Labels for computing classification loss.
            y_char = y_char.to(self.device)     # Labels for computing classification loss.
            ...
            d_loss_cls = self.classification_loss(out_cls, x_char, self.dataset)
